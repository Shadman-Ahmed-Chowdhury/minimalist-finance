...
use function Livewire\Volt\{state, form, computed, mount};
...
mount(function ($transfer) {
    $this->form->setTransfer($transfer);
});

$save = function () {
    try {
        $validate = $this->form->update();
        if ($validate) {
            $this->showModal = false;
            Toaster::success('Transfer updated successfully');
            $this->dispatch('transferUpdated' . $this->transfer->id);
        }
    } catch (\Exception $th) {
...
<x-dialog wire:model="showModal">
    <x-dialog.open>
        <button title="Edit" class="px-1 py-1 text-primary-500">
